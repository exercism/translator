...
This glossary was forked from Jiki's Simplified Chinese glossary when Exercism's translator repo was
...
characters and physical metaphors) went with the fork; every decision about the Simplified Chinese
...
Simplified Chinese has been asked yet, and an agent's invented rendering would read as an agreed one.
...
- `scenario` → 场景. Named a structured variant within an exercise.
- `pathway` → 学习路径.
- `bootcamp` → 训练营.
- `track (a curriculum path, a different thing from Exercism's track)` → 学习路线.
