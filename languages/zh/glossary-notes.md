...
This family glossary was forked from Jiki's Chinese family glossary when Exercism's
...
about the Chinese language itself was kept, including the ones a native speaker made. Where a
...
because the product's nouns are shared across the family. Nobody who speaks Chinese has been
...
- `character (an on-screen figure the reader controls)` → 角色. Not 小人, which reads as
  "a petty or mean person", or as an old humble first-person pronoun, rather than a
  friendly figure. Distinct from the text-character sense (字符 in zh-CN's glossary), so
  no collision. Identical in both scripts.
