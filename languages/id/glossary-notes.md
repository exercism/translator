...
| hint | petunjuk | Keeps _instruksi_ free for instructions someone is given. | medium |
...
- **Why "kamu" and not "Anda".** "Kamu" is the standard register of Indonesian
  ...
  address the reader this way); it reads warm and direct and matches the mentor voice.
...
- **Why the audience register is the tutorial one.** Indonesian dev-education publishers
  ...
  tutor voice this content wants, so their register is the model rather than the more formal
...
alternative, a collision, a register distinction, an English-loanword decision, or the metaphor set were all left untouched.
...
Two decisions came out of that pass that bind Indonesian:
...
- **Rows added for the concept page**, including `instruction`,
...
| Term | Why |
|------|-----|
| instruction | Everyday naturalized word, the one Indonesian tutorials use for "instruksi program". |
...
| course | Dicoding uses `kelas` for a course, but that word is already taken by the OOP `class` row, so Indonesian uses `kursus` instead ("Di kursus ini" for "in this course"). |
