...
## Term clarification (Hebrew realisation)

The principle (the `<define>`/`<literal>` markup, the no-auto-gloss rule, and direction from
the glossary's `Use (he/en)` column) lives in `global/voice.md`. This section gives only the
Hebrew specifics for expanding a `<define>`.

- The Hebrew marker for "in English" is **`באנגלית:`**, placed before the English term inside
  the brackets. Never the literary `בלעז`.
- **Hebrew-primary term** (`Use = he`): Hebrew first, English in italics after `באנגלית:`.
  `פונקציה (באנגלית: _function_)`, `משתנה (באנגלית: _variable_)`.
- **English-primary term** (`Use = en`): English first in italics, Hebrew gloss in brackets,
  no marker word. `_API_ (ממשק תכנות יישומים)`.
- **Code identifier**: code span, Hebrew meaning in brackets, no marker word.
  `` `turnLeft` `` (פנה שמאלה).
- The italic applies only to the Latin-script English term. Do not italicise Hebrew.

## The name "Jiki"

- **Keep `Jiki` in Latin script throughout Hebrew prose**, and give it **no definite
  article**: `Jiki`, never `ה־Jiki`.
- **Attach prefixes with a maqaf**: `ב־Jiki`, `ל־Jiki`, `ו־Jiki`. Use `של` for possession
  (`הקוד של Jiki`), not a construct chain.
- **Introduce the pronunciation once**, at the very first mention in a piece, as
  `Jiki (ג'יקי)`. After that, always bare `Jiki`. Never switch to `ג'יקי` as the running
  form.
- **Refer back with a pronoun (`הוא`) after the first mention in a passage, rather than
  repeating `Jiki` in every sentence.** Native Hebrew prose names its subject once and then
  carries it with pronoun and verb agreement, the same way it treats any other repeated
  subject. Do not chop a paragraph into short one-clause sentences that each restate the
  name (`זה Jiki. Jiki הולך להיות...`); that staccato reads as machine output, not warm
  prose. A short sentence is still fine where the content genuinely calls for a beat — a
  dialogue line, a pause for a picture, a deliberate punchline — just not as the default
  rhythm.
- Verb agreement follows the implied Hebrew category noun.
