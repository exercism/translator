...
This glossary was forked from Jiki's Hebrew glossary when Exercism's translator repo was
...
characters and physical metaphors) went with the fork; every decision about the Hebrew
...
Hebrew has been asked yet, and an agent's invented rendering would read as an agreed one.
...
- `track / pathway` → מסלול. Not `נתיב`, which reads as a file path.
...
- **The gloss marker is `באנגלית:`**, the Hebrew Wikipedia convention (present in thousands of articles), rather than the literary `בלעז`.
