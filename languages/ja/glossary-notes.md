...
This glossary was forked from Jiki's Japanese glossary when Exercism's translator repo was
...
characters and physical metaphors) went with the fork; every decision about the Japanese
...
Japanese has been asked yet, and an agent's invented rendering would read as an agreed one.
...
- `pathway` → 学習パス. A structured learning sequence; near-synonym of "track".
- `bouncer (club/bar doorkeeper)` → バウンサー. Never 門番, which is a gatekeeper guarding the
  gate of a castle.
