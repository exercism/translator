# Catalan (ca) glossary notes

## 2026-09-20: Adapted for Exercism (agent; nothing here is a native speaker's decision)

This glossary was forked from Jiki's Catalan glossary when Exercism's translator repo was
...
characters and physical metaphors) went with the fork; every decision about the Catalan
...
Catalan has been asked yet, and an agent's invented rendering would read as an agreed one.
...
- `pathway` → camí. Kept distinct from _itinerari_ (track) even though they're near-
  synonyms in English too.
- `track (a curriculum path, a different thing from Exercism's track)` → itinerari. The
  curriculum path a learner follows. Never _recorregut_.
- `bootcamp` was kept in English, as transparent to a Catalan reader.
