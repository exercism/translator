...
This glossary was forked from Jiki's European Spanish glossary when Exercism's translator repo was
...
characters and physical metaphors) went with the fork; every decision about the European Spanish
...
European Spanish has been asked yet, and an agent's invented rendering would read as an agreed one.
...
- `pathway` → itinerario. _Itinerario formativo_ is the fuller form. Near-synonym of
  _track_.
- `track (a curriculum path, a different thing from Exercism's track)` → ruta (de
  aprendizaje).
