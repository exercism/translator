...
- **An interpolated value takes no case ending and no fixed article.** A UI string or an
  ...
  - **Never attach a case ending to the placeholder**: it will be wrong for half the values.
    Rephrase so that a Greek noun beside it carries the ending: not "in `%{track_title}`",
  ...
  - After a numeral, use the form Greek normally uses with a counted noun, whatever the
    number turns out to be.
- **The exact forms this produces still want a native speaker's eye**, since no Greek
