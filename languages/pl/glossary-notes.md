...
This glossary was forked from Jiki's Polish glossary when Exercism's translator repo was
...
characters and physical metaphors) went with the fork; every decision about the Polish
...
Polish has been asked yet, and an agent's invented rendering would read as an agreed one.
...
- `character (an on-screen figure)` → ludzik. **Masculine**, so every agreeing verb,
  adjective, participle and pronoun is masculine. Declines _ludzika_ (gen./acc.),
  _ludzikowi_ (dat.). Never _postać_, which is feminine and too abstract for a little
  figure.
- `turn (rotating on the spot)` → obracać się (verb) / obrót (noun). The reflexive _się_
  is required on the verb. Never _skręcać_ / _skręcić_ / _skręt_, which describe turning
  while travelling and imply moving forward through the turn.
