...
This glossary was forked from Jiki's Croatian glossary when Exercism's translator repo was
...
characters and physical metaphors) went with the fork; every decision about the Croatian
...
Croatian has been asked yet, and an agent's invented rendering would read as an agreed one.
...
- `scenario` → scenarij. Croatian spelling; never `scenario`.
- `bootcamp` was kept in English, as Croatian tech-education copy does.
