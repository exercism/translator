---
description: Translate the analyzer comments (exercism/website-copy) into one language
...
- English: `exercism/website-copy`, `analyzer-comments/**` and nothing else. **This is not the
  website's UI strings**: those are `/translate-website`. The name is the repo's.
- Type: `analyzer-comments` (`content-types/analyzer-comments.md`).
- Output: `../i18n/locales/$locale/content/...` by blob id.
...
2. The real run, **in the background** (`run_in_background`), exactly this and nothing added:
   ...
   ```
   node scripts/translate.mjs website-copy $locale
   ```
