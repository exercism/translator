...
| canvas | _area di disegno_ | The drawing surface in graphics code. Where the English itself explains the term, explain it as the _area di disegno_ (the `canvas` element), then use _canvas_. Where it does not, never leave a bare _canvas_: use _area di disegno_. |
