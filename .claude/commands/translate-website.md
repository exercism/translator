---
description: Translate the website's two UI catalogs (Rails and i18next) into one language
...
- English: `exercism/website`, the Rails YAML under `config/locales/` and the i18next bundles
  under `app/javascript/i18n/en/`, flattened by the `i18n` repo into two catalogs.
- Types: `website-backend` (`content-types/website-backend.md`) and `website-frontend`
  (`content-types/website-frontend.md`).
- Output: `../i18n/locales/$locale/website/{backend,frontend}.json`, and the per-unit stamps
  the `i18n` checker writes beside them. These two are key-based: a unit the locale lacks is
  translated, and a unit whose English was edited since it was stamped is retranslated.
...
2. The real run, **in the background** (`run_in_background`), exactly this and nothing added:
   ...
   ```
   node scripts/translate.mjs website $locale
   ```
